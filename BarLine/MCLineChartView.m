...
#define DOT_PADDING_DEFAULT 150
...
- (void)initialization {
    ...
    _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(LINE_CHART_LEFT_PADDING, 0, width -  LINE_CHART_TOP_PADDING, CGRectGetHeight(self.bounds))];
...
//    if (!_hideYAxis) {
//        int a = (self.frame.size.width - 50) / 7;
//        for(int i =0 ;i <= 7; i++){
//            CGContextSetStrokeColorWithColor(context, _colorOfYAxis.CGColor);
//            CGContextMoveToPoint(context, (LINE_CHART_LEFT_PADDING - 1) +  i * a, LINE_CHART_TOP_PADDING - 1);
//            CGContextAddLineToPoint(context,( LINE_CHART_LEFT_PADDING - 1 )+ i *a, LINE_CHART_TOP_PADDING + _chartHeight + 1);
...
//    }
    ...
    for(int i = 0; i <= _numberOfYAxis ; i++){
        ...
        CGContextMoveToPoint(context, LINE_CHART_LEFT_PADDING - 1, LINE_CHART_TOP_PADDING + _chartHeight + 1 - unitHeight *i);
        CGContextAddLineToPoint(context, width - LINE_CHART_RIGHT_PADDING + 1, LINE_CHART_TOP_PADDING + _chartHeight + 1 - unitHeight *i);
        CGContextStrokePath(context);
    }
...
//    CGFloat maxHeight = [self.maxValue floatValue];
   CGFloat value = [rawHeight floatValue] - [self.minValue floatValue];
   CGFloat maxHeight = [self.maxValue floatValue] - [self.minValue floatValue];
...
- (void)reloadChartYAxis {
    ...
    CGFloat unitHeight = _chartHeight/_numberOfYAxis;
    double unitValue = ([self.maxValue floatValue] - [_minValue floatValue])/_numberOfYAxis;
    for (NSInteger i = 0; i <= _numberOfYAxis; i ++) {
        UILabel *textLabel = nil;
        if(unitValue >=10000 && unitValue < 1000000){
            textLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, chartYOffset - 10, LINE_CHART_LEFT_PADDING + 15, 20)];
        }else if(unitValue >= 1000 && unitValue < 10000){
        textLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, chartYOffset - 10, LINE_CHART_LEFT_PADDING + 5, 20)];
        }else if (unitValue >= 10000000){
             textLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, chartYOffset - 10, LINE_CHART_LEFT_PADDING + 30, 20)];
        }
        else{
              textLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, chartYOffset - 10, LINE_CHART_LEFT_PADDING - 2, 20)];
        }
        if([self.minValue floatValue] > 100000){
            textLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, chartYOffset - 10, LINE_CHART_LEFT_PADDING + 20, 20)];
        }
        ...
        }else if (unitValue> 0.1 && unitValue <= 1){
             textLabel.text = [NSString stringWithFormat:@"%.2f%@", unitValue * i + [_minValue floatValue], _unitOfYAxis];
        }else if (unitValue <= 0.1){
             textLabel.text = [NSString stringWithFormat:@"%.2f%@", unitValue * i + [_minValue floatValue], _unitOfYAxis];
        }
        if([_minValue floatValue] == 40){
             textLabel.text = [NSString stringWithFormat:@"%.0f%@", unitValue * i + [_minValue floatValue], @""];
        }
        if(self.numberOfYAxis == 3){
            if(unitValue * i + [_minValue floatValue] == 0){
                textLabel.text = [NSString stringWithFormat:@"%.0f%@", unitValue * i + [_minValue floatValue], @""];
            }else if(unitValue * i + [_minValue floatValue] == 150){
...
//            });
            ...
            //直线图
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(array.count * 0.4 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                lineLayer.fillColor = _solidDot ? [UIColor clearColor].CGColor : [UIColor clearColor].CGColor;
            });
        }
    }
}
