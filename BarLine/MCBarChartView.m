...
//    _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(BAR_CHART_LEFT_PADDING, 0, width - BAR_CHART_RIGHT_PADDING - BAR_CHART_LEFT_PADDING, CGRectGetHeight(self.bounds))];
    _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(BAR_CHART_LEFT_PADDING, 0, [UIScreen mainScreen].bounds.size.width - BAR_CHART_LEFT_PADDING, CGRectGetHeight(self.bounds))];
    NSLog(@"%f__%f",self.bounds.size.height,self.bounds.size.width);
    NSLog(@"%f__%f",[UIScreen mainScreen].bounds.size.width,[UIScreen mainScreen].bounds.size.height);
...
//    CGContextAddLineToPoint(context, width , BAR_CHART_TOP_PADDING + _chartHeight + 1);
//    CGContextStrokePath(context);
    
    for(int i = 0; i <= _numberOfYAxis; i++){
        CGFloat unitHeight = _chartHeight/_numberOfYAxis;
        CGContextMoveToPoint(context, BAR_CHART_LEFT_PADDING - 1, BAR_CHART_TOP_PADDING + _chartHeight + 1 - unitHeight *i);
        CGContextAddLineToPoint(context, width + 1, BAR_CHART_TOP_PADDING + _chartHeight + 1 - unitHeight *i);
        CGContextStrokePath(context);
    }
}
...
- (CGFloat)normalizedHeightForRawHeight:(NSNumber *)rawHeight {
    CGFloat value = [rawHeight floatValue] - [self.mixValue floatValue];;
    CGFloat maxHeight = [self.maxValue floatValue] - [self.mixValue floatValue];
...
//   CGFloat contentWidth = ((self.bounds.size.width - BAR_CHART_RIGHT_PADDING - BAR_CHART_LEFT_PADDING) - (_barWidth * _sections)) / (_sections + 1);
...
//    _scrollView.contentSize = CGSizeMake(self.frame.size.width - 45, 0);
...
- (void)reloadChartYAxis {
    ...
    CGFloat unitHeight = _chartHeight/_numberOfYAxis;
    CGFloat unitValue = ([self.maxValue floatValue] - [self.mixValue floatValue])/_numberOfYAxis;
...
//        UILabel *textLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, chartYOffset - unitHeight * i - 10, BAR_CHART_LEFT_PADDING - 2, 20)];
        ...
        if(unitValue > 10000 && unitValue <= 1000000){
            ...
            textLabel.frame = CGRectMake(1, chartYOffset - unitHeight * i - 10, BAR_CHART_LEFT_PADDING + 10, 20);
        }else if(unitValue > 1000000){
            textLabel.textAlignment = NSTextAlignmentLeft;
            textLabel.frame = CGRectMake(1, chartYOffset - unitHeight * i - 10, BAR_CHART_LEFT_PADDING + 15, 20);
        }else{
        ...
        if(unitValue <1){
            textLabel.text = [NSString stringWithFormat:@"%.2f%@", unitValue * i + [_mixValue floatValue], _unitOfYAxis];
        }else{
        textLabel.text = [NSString stringWithFormat:@"%.0f%@", unitValue * i + [_mixValue floatValue], _unitOfYAxis];
        }
...
//                UILabel *textLabel = [[UILabel alloc] initWithFrame:CGRectMake(xSection - _paddingSection/2, _chartHeight + BAR_CHART_TOP_PADDING, xOffset - xSection + _paddingSection, BAR_CHART_TEXT_HEIGHT)];
                UILabel *textLabel = [[UILabel alloc] initWithFrame:CGRectMake(xOffset - 26, _chartHeight + BAR_CHART_TOP_PADDING, 56, BAR_CHART_TEXT_HEIGHT)];
                textLabel.textColor = _colorOfXText;
                textLabel.textAlignment = NSTextAlignmentCenter;
                textLabel.font = [UIFont systemFontOfSize:14];
                textLabel.numberOfLines = 0;
                textLabel.text = [self.dataSource barChartView:self titleOfBarInSection:index];
